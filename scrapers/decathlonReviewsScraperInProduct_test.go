...
func TestReviewScraperDecathlon(t *testing.T) {
	e := godotenv.Load()
	if e != nil {
		fmt.Print(e)
	}
	config := models.ScrapingConfig{UrlBase: "http://localhost:8000", ScraperId: "testScraperElpais", DeviceID: "testDeviceElpais"}
	scraper := DecathlonReviewsScraperInProduct{Config: config}
	date := time.Date(2018, 05, 04, 0, 0, 0, 0, time.UTC)

	urlNew := UrlNew{url: "https://www.decathlon.es/es/p/bicicleta-de-montana-rockrider-st-100-27-5-gris/_/R-p-192872?mc=8400335&c=GRIS", date: date}
